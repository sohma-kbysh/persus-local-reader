...
インストールする作業も必要ありません。ここでは、初めて使う方向けに
少し丁寧に手順を説明します。

Githubからのdownloadは、右上にある緑の「Code」とかいてあるボタンを押して、download ZIPからできます。ZIPをダウンロードしたら、documentや任意の作業フォルダでZIPを展開してください。

### このアプリでできること

プラトン『ソクラテスの弁明』のギリシア語原文を画面で読みながら、わからない
単語をクリックするだけで、その単語がどんな形（格・数・時制・人称など）で
使われているのかがすぐに表示されます。紙の辞書や文法書をその都度引く手間を
減らすための道具です。
...
「Open Plato Apology」とだけ表示されることがあります。同じアプリです。）

```text
Open Plato Apology.app
```

ダブルクリック（または右クリックで詳細を表示して開くを選択）すると、あなたのパソコンの中だけで小さなプログラムが動き出し
（インターネット上のどこかにあるサーバーに接続するのではなく、いわば
あなたのパソコンの中に小さな図書館を一時的に開くようなイメージです）、
数秒後に自動的にブラウザ（Safari や Chrome など、ふだんウェブサイトを見る
のに使っているアプリ）が開きます。何もアップロードされることはなく、
すべての処理はあなたのパソコンの中だけで完結します。
...
ブラウザにギリシア語の本文が表示されます。読み進めながら、意味や形が
わからない**ギリシア語の単語をクリック**してみてください。画面の一部に
小さなパネルが開き、その単語の見出し語（辞書に載っている基本形）や、
...
また読みたくなったら、もう一度 `Open Plato Apology.app` をダブルクリック
...
読書環境は `http://127.0.0.1:8000/` というアドレス（多分、だいたいこれなはず）で、あなたのパソコンの中だけで動作します。外部のサーバーには一切データを送信しません。
